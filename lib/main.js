...
import {createLibraryResolver, getImportedSymbols} from './imports.js';
...
import {getWorkspaceRename} from './rename.js';
 ...
 * @param {string} uri
 * @param {import('scarpet-parser').Node} root
 * @returns {Promise<Map<string, import('./imports.js').LibraryResolution>>}
 */
function collectImportedSymbols(uri, root) {
    return getImportedSymbols(uri, root, {
        getWorkspaceFolderPaths: () => workspaceFolderPaths,
        loadAst,
    });
}
...
connection.onHover(async ({textDocument, position}) => {
    ...
    return getHoverContents(
        ...
        {getWorkspaceFolderPaths: () => workspaceFolderPaths, loadAst},
    );
});
...
connection.onRenameRequest(async ({textDocument, position, newName}) => {
    ...
    if (document === undefined || ast === undefined) return undefined;
    try {
        const node = getNodeAt(ast, document.offsetAt(position));
        const workspaceRename = await getWorkspaceRename(
            textDocument.uri,
            ast,
            node,
            newName,
        ...
        if (workspaceRename === undefined) return undefined;
        return workspaceRename;
    } catch (error) {
        connection.console.error(`Error in rename: ${String(error)}`);
