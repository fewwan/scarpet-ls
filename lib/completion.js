...
export function pushFunctions(
    ...
    for (const declaration of declarations) {
        ...
        if (
            ...
            declaration.docComment !== undefined
        ) {
...
/**
 * Push completion items for symbols provided by library imports.
 *
 * @param {boolean} snippetSupport
 * @param {boolean} useItemDefaults
 * @param {boolean} useMarkdown
 * @param {CompletionItem[]} items
 * @param {Map<string, import('./imports.js').LibraryResolution>} symbols
 * @param {Range} range
 */
export function pushLibraryFunctions(
    snippetSupport,
    useItemDefaults,
    useMarkdown,
    items,
    ...
    for (const [name, symbol] of symbols) {
        ...
        if (
            ...
            item.detail = getFunctionSyntax(declaration);
            if (
                declaration.comment !== undefined ||
                declaration.docComment !== undefined
            ) {
 ...
 * ) => Promise<Map<string, import('./imports.js').LibraryResolution>>} collectImportedSymbols
 * @param {string} uri
 * @returns {Promise<CompletionList | undefined>}
 */
export async function getCompletion(
    ...
    const symbols = await collectImportedSymbols(uri, root);
    pushLibraryFunctions(
        snippetSupport,
        useItemDefaults,
        useMarkdown,
        items,
