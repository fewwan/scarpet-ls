...
/**
 * Deterministic candidate locations for a module file.
 *
 * @param {string} dir
 * @param {string} module
 * @returns {string[]}
 */
function candidatePaths(dir, module) {
    /** @type {string[]} */
    const result = [];
    for (const ext of LIBRARY_EXTENSIONS) {
        result.push(path.join(dir, `${module}.${ext}`));
        result.push(path.join(dir, module, `${module}.${ext}`));
    }
    return result;
}
 ...
 *
 * Tries, in order: `<dir>/<module>.sc`, `<dir>/<module>.scl`,
 * `<dir>/<module>/<module>.sc`, `<dir>/<module>/<module>.scl`, then a bounded
 * recursive search for `<module>.sc`/`<module>.scl`.
 *
 * @param {string} dir
 * @param {string} module
...
async function findModuleFile(dir, module) {
    for (const candidate of candidatePaths(dir, module)) {
