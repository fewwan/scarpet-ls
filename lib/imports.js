...
/**
 * Check whether a node is the module name parameter of an import call.
 *
 * @param {import('scarpet-parser').Node} root
 * @param {import('scarpet-parser').Node} node
 * @returns {LibraryImport | undefined}
 */
export function findImportModuleParam(root, node) {
    for (const imp of getImports(root)) {
        if (imp.moduleNode === node) return imp;
    }
    return undefined;
}

/**
 * Check whether a node is a symbol name parameter of an import call.
 *
 * @param {import('scarpet-parser').Node} root
 * @param {import('scarpet-parser').Node} node
 * @returns {LibraryImport | undefined}
 */
export function findImportSymbolParam(root, node) {
    for (const imp of getImports(root)) {
        for (const param of imp.node.params.slice(1)) {
            const resolved = resolveExpression(param);
            if (resolved === node) return imp;
        }
    }
    return undefined;
}
